...
class AgentSessionAggregate:
    ...
    Implements the Gas Town pattern:
    - First event MUST be AgentContextLoaded
    - All decisions must reference the loaded context
    ...
    async def load(cls, store: EventStore, agent_id: str, session_id: str) -> "AgentSessionAggregate":
        ...
        This enables the Gas Town pattern - agent can crash and recover
    ...
    def assert_context_loaded(self) -> None:
        ...
        if not self.context_loaded:
            raise DomainError(
                f"Agent session {self.agent_id}/{self.session_id} has no context loaded",
    ...
    def assert_model_version_current(self, expected_version: str) -> None:
        ...
        if self.model_version != expected_version:
            raise DomainError(
                f"Expected model version {self.model_version}, got {expected_version}",
                aggregate_type="AgentSession",
                stream_id=f"agent-{self.agent_id}-{self.session_id}"
            )